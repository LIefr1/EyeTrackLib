...
class Trainer:
    def __init__(
        ...
        log_filename = f"logs/train_log_{self.model_name}-{self.log_time}.log"
    ...
    def _print_overwrite(self, step, total_step, loss, operation):
        self.logger.info("\r")
        if operation == "train":
            self.logger.info("Train Steps: %d/%d  Loss: %.4f " % (step, total_step, loss))
        else:
            self.logger.info("Valid Steps: %d/%d  Loss: %.4f " % (step, total_step, loss))
        ...
        for epoch in trange(
            ...
            with tqdm(
                ...
                for step, (images, landmarks) in enumerate(train_loader):
                    ...
                    pbar_train.set_postfix({"Train Loss": f"{running_loss:.4f}", "Epoch": epoch})
                    pbar_train.update()
            ...
            if loss_valid < loss_min:
                loss_min = loss_valid
                torch.save(model.state_dict(), f"models/{self.model_name}-{self.log_time}.pth")
                self.logger.info(
